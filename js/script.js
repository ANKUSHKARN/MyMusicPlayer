...
async function displayAlbums(){
    ...
    let array = Array.from(anchors)
        ...
        if(e.href.includes("/songs") && !e.href.includes(".htaccess")) {
            ...
            <img style="aspect-ratio:1/1" src="/songs/${folder}/cover.jpg" alt="">
...
async function main() {
    ...
    next.addEventListener("click", () => {
        ...
        if ((index + 1) < songs.length) {
            ...
            alert("This is the last song of the list")
        }
    })


    header_previous.addEventListener("click", () => {
        currentSong.pause()
        let index = songs.indexOf(currentSong.src.split("/").slice(-1)[0])
        if ((index-1) >= 0) {
            playMusic(songs[index - 1])
    ...
    header_next.addEventListener("click", () => {
        currentSong.pause()
        let index = songs.indexOf(currentSong.src.split("/").slice(-1)[0])
        if ((index + 1) < songs.length) {
            playMusic(songs[index + 1])
...
document.querySelector("#showAlertButton").addEventListener("click", function() {
...
document.querySelector("#showAlertButton1").addEventListener("click", function() {
    alertBox.style.display = "block"; 
    setTimeout(function() {
      alertBox.style.display = "none"; 
    }, 3000);
